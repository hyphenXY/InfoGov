...
cursor.execute("desc GST_Registration")
x = cursor.fetchall()
sql_fields = [i["Field"] for i in x]
cursor.close()
print(sql_fields)

df1 = pd.read_csv("etl/trans_register_payers_per_year_data.csv",index_col=0)
...
for _, i in df1.iterrows():
    try:
        cursor = connection.cursor()
        print("insert into GST_Registration ("+', '.join(sql_fields[:])+") values ("+ ', '.join(["%s" for i in sql_fields[:]]) +")", tuple(i[sql_fields].to_list()))
        cursor.execute(
            "insert into GST_Registration ("+', '.join(sql_fields[:])+") values ("+ ', '.join(["%s" for i in sql_fields[:]]) +")",
            tuple(i[sql_fields].to_list())
        )
        connection.commit()
        cursor.close()
    except:
